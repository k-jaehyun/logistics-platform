...
import com.logistics.platform.slack_service.domain.model.QSlack;
import com.logistics.platform.slack_service.presentation.response.QSlackResponseDto;
...
public class SlackRepositoryCustomImpl implements SlackRepositoryCustom {

  private final JPAQueryFactory queryFactory;

  public SlackRepositoryCustomImpl(EntityManager em) {
    this.queryFactory = new JPAQueryFactory(em);
  }

  @Override
  public Page<SlackResponseDto> findAllToPage(
    ...
    if (uuidList != null && !uuidList.isEmpty()) { // idList 값이 있다면 조회
      builder.and(slack.id.in(uuidList));
    }
    builder.and(slack.isDeleted.eq(false)); // isDeleted=false 만 조회
    ...
    List<SlackResponseDto> results = queryFactory
        .select(new QSlackResponseDto(slack)) // 원래 SlackResponseDto로 들어가야하는데 뭔가 오류가...
        .from(slack)
        .where(builder)
        .orderBy(getDynamicSort(pageable.getSort(), slack.getType(), slack.getMetadata()))
